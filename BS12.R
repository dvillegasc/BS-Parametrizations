BS12 <- function(mu.link = "log", sigma.link = "log"){
  mstats <- checklink("mu.link", "dBS12", substitute(mu.link),
                      c("log", "inverse", "identity", "own"))
  dstats <- checklink("sigma.link", "dBS12", substitute(sigma.link),
                      c("log", "logit", "probit", "own"))
  structure(
    list(family = c("dBS12", "Birnbaum-Saunders - twelfh parameterization"),
